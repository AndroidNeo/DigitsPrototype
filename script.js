...
function addLevelsElements() {

    for (var i = 1; i <= levelCount; i++) {
        var idStr = "level_item_" + i;
        idStr = " id = '" + idStr + "'";
        $("#main_screen").append("<div class='level_item'" + idStr + ">" + i + "</div>");
    }

    for(var i = 1; i <= levelCount; i++) {
        $("#level_item_" + i).click(function (eventObject) {

            var idStr = eventObject.toElement.id;
            idStr = idStr.replace("level_item_","");
            var selectedLevel = parseInt(idStr, 10);

            runLevelWindowByNumber(selectedLevel);
...
function runLevelWindowByNumber(selectedLevel) {
